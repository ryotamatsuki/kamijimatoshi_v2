...
- [x] **企業版ふるさと納税の優位性**
    ...
    - [ ] 上島町における公式な受け入れ体制 (上島町企業版ふるさと納税実施要綱) - *contact.htmlにリンク記述*
    - [ ] ベンチマーキングと潜在的可能性 (松山市18億円、砥部町1,544万円の実績) - *コンテンツ未配置*
...
- [x] **包括的な企業立地インセンティブ**
    ...
    - [x] 各奨励措置の具体的な内容
        - [x] **情報通信関連企業奨励**: 年額の1/3以内、上限1,000万円/年 (3年間) - *index.htmlに概要記述*
        - [x] **雇用促進奨励**: 新規常用雇用従業員1人につき30万円 (3年間、総額3,000万円限度) - *コンテンツ未配置*
        - [x] **新規創業者等応援**: 補助率2/3、上限400万円 - *コンテンツ未配置*
        - [x] **企業立地促進**: 投下固定資産額の10% (上限5,000万円)、上下水道料金50%補助 (3年間、上限1,000万円)、固定資産税3年間減免 - *コンテンツ未配置*
    - [ ] 競合分析と戦略的ポジショニング (松山市、大崎上島町との比較、情報通信関連企業への手厚さ強調) - *コンテンツ未配置*
- [x] **官民連携（PFI/PPP）プロジェクトへの新たな投資機会**
    - [ ] 国および地域の動向 (ウォーターPPP、愛媛県内のサウンディング型市場調査、西伊予市PFI事例) - *コンテンツ未配置*
...
- [x] **定量化されたESG/SDGsへの貢献**
    ...
    - [x] **社会 (S)**:
        - [x] 弓削高校魅力化プロジェクト: SDGs目標4「質の高い教育をみんなに」、目標11「住み続けられるまちづくりを」への貢献 (公営塾、学生寮、全国からの生徒誘致) - *investment.htmlにカード追加*
...
- [x] **人口動態と地域活性化への取り組み**
    ...
    - [ ] 「上島モデル」と直島モデルの比較分析 (多様性、住民中心、リスク分散) - *コンテンツ未配置*
...
- [x] **オープンデータと一次情報へのアクセス**
    - [x] 条例・要綱への直接リンク (上島町企業誘致促進条例、上島町企業版ふるさと納税実施要綱) - *data.htmlにリンク記述*
    - [x] 総合計画・個別計画への直接リンク (上島町第2次総合計画、上島町地球温暖化対策実行計画、浜の活力再生プラン) - *data.htmlにリンク記述*
    - [x] CSV形式でのデータダウンロード機能の要件定義 (KPIダッシュボードの生データ) - *data.htmlにボタン記述*
    - [ ] 年次報告書、FAQの準備 (専門的な質問への回答) - *コンテンツ未配置*
...
- [x] **タイポグラフィ・システム**
    ...
    - [x] **フォントウェイト、サイズ、行間、字間の定義** (デスクトップ/モバイル両方) - *style.cssに記述*
        ...
        - [x] **セクション見出し (H2)**: Clash Display Medium, 48px/32px, Line Height 1.2, Letter Spacing -1.5% - *style.cssに記述*
        ...
        - [x] **ボタンテキスト (Button)**: Satoshi Medium, 16px/15px, Line Height 1.0, Letter Spacing +2% - *style.cssに記述*
        - [x] **ラベル/キャプション (Label)**: Geist Mono Regular, 12px/11px, Line Height 1.4, Letter Spacing +1% - *style.cssに記述*
    - [x] **視覚的階層 (ヒエラルキー) の設計** (Studio Formatの例を参考に) - *style.cssに記述*
    - [ ] **バリアブルフォントの採用検討** (パフォーマンスと表現力 - 今回はGoogle Fontsで代替) - *検討済み、代替フォント使用*
- [x] **カラーパレット**
    ...
    - [ ] **ダークモード対応の検討** (CSS変数での実装、ユーザー選択可能にするかデフォルトにするか) - *検討済み、未実装*
- [x] **レイアウトとグリッド**
...
- [x] **モーション・インタラクション**
    ...
    - [ ] **カスタムカーソルの実装**
        - [ ] subtle (繊細な) カスタムカーソル (通常は小さなドット、クリック可能要素へのホバー時に形を変える - 小さな円や矢印) - *未実装*
...
- [x] **`index.html` (トップページ)**
    - [x] **ヒーローセクション**
        - [x] 高品質な背景画像/動画 (上島町の風景、産業、人々の活動 - 「投資誘致Webページ最新デザイントレンド.md」の「ヒーローセクション」参照) - *プレースホルダー画像使用中*
...
- [x] **`investment.html` (投資案件カタログ)**
    ...
    - [x] **フィルタリング機能のUI**
        - [x] テーマ (DX推進、環境保全、教育、地域活性化、起業支援など) - *セレクトボックスのプレースホルダーのみ*
    ...
    - [ ] **各案件の詳細ページ** (必要に応じて別ファイルまたはモーダル/アコーディオンで実装) - *未実装*
        - [ ] 案件名 (`<h1>`) - *未実装*
        - [ ] 課題 (The Challenge): なぜこのプロジェクトが必要なのか (例: 弓削高校の廃校危機) - *未実装*
        - [ ] 解決策 (Our Solution): 具体的な取り組み内容 (例: 公営塾の設置、学生寮の整備) - *未実装*
        - [ ] インパクト (Expected Impact) と関連KPI (例: 町外からの入学者数12名確保) - *未実装*
        - [ ] 企業へのメリット (Value for Partners): 税制優遇、ESG/SDGsへの貢献、PR効果など) - *未実装*
        - [ ] 関連画像/動画 - *未実装*
        - [ ] 問い合わせCTA - *未実装*
...
- [x] **`about.html` (上島町について)**
    ...
    - [x] **各セクションのコンテンツ**
        ...
        - [x] **観光とレジャー**: 主要観光地、マリンスポーツ、食、宿泊施設 (多様な選択肢) - *記述済み*
...
- [x] **`data.html` (データと実績)**
    ...
    - [x] **オープンデータと一次情報へのアクセスセクション**
        - [x] 条例・要綱、総合計画・個別計画への直接リンク (上島町企業誘致促進条例、上島町企業版ふるさと納税実施要綱、上島町第2次総合計画、上島町地球温暖化対策実行計画、浜の活力再生プラン、上島町まち・ひと・しごと創生推進計画) - *リンク記述済み*
        - [x] **CSV形式でのデータダウンロードボタン** (KPIダッシュボードの生データ) - *ボタン記述済み*
        - [ ] 年次報告書、FAQの準備 (専門的な質問への回答) - *コンテンツ未配置*
...
- [x] **各セクションのスタイル**
    ...
    - [x] ナビゲーションのスタイリング (デスクトップ、モバイルメニューの開閉アニメーション)
    ...
    - [x] フォーム要素のスタイリング (入力フィールド、ボタン、チェックボックス)
...
- [x] **アクセシビリティ (WCAG 2.2 AAレベル準拠)**
    ...
    - [x] フォーカスインジケーターの視認性確保 (`:focus`スタイル - デフォルトのoutlineを維持または強化) - *デフォルトのブラウザスタイルに依存*
...
- [x] **フォーム・データ表示**
    ...
    - [x] 税控除シミュレーターのロジック実装
        ...
        - [x] 計算処理 (損金算入、税額控除のロジックを正確に実装)
        - [x] 結果の動的な表示 (税の軽減効果合計額、実質的な企業負担額)
    - [ ] KPIスナップショットの動的更新 (もしあれば、プレースホルダーから実際の値への置き換え、アニメーション表示) - *未実装*
...
- [ ] **画像・動画**
    - [ ] **ヒーローセクションの背景画像/動画の選定と最適化** (ファイルサイズ、フォーマット - WebP/MP4、解像度) - *プレースホルダー画像使用中*
